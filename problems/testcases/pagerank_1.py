...
def prepareProblem(*, algorithm_params: AlgorithmParams = AlgorithmParams()):
    ...
    G: nx.DiGraph = nx.readwrite.read_edgelist(f'ca_search.edges', create_using=nx.DiGraph)
    n = G.number_of_nodes()
    GraphMatr: np.ndarray = np.array(nx.google_matrix(G)).T
    ...
    algorithm_params.lam = 0.6
    # for Bregman variants
    algorithm_params.lam_KL = 0.75
    ...
    if real_solution is not None:
        print(f"Second test on real solution: {res.F(real_solution)}")
